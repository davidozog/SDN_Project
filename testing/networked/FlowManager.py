...
class Host:
  ...
  def printSetData(self):
    ...
    print "\n"
  def elementsOnHost(self):
    sum=0  
    for key in self.myDataSets.keys():
      sum+=len(self.myDataSets[key].myElements.keys())
    return sum
  ...
  def request(self,host,setName,element):
    self.myManager.requestsSinceUpdate+=1
    ...
    if r <  self.myProbabilities[setName] :
      ...
      if(self.elementsOnHost()+host.elementsOnHost()<>200):
        print "RUHROH, DATA (DE)MATERIALIZING"
        pdb.set_trace()    
    else:
